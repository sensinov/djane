...
function getClient (callback) {
    if (!connectedClientToDb) {
        console.log('Trying to connect to DB', config.mongoDBHost); 
        MongoClient.connect(config.mongoDBHost, {useNewUrlParser: true}, function (err, client){
            if (err) {
                console.error(err); 
                return callback(err); 
            }
            connectedClientToDb = client.db(config.DataBaseName); 
            console.log('Conncected to DB: ' + config.mongoDBHost); 
